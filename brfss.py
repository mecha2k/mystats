...
def MakeNormalModel(weights):
    ...
    weights: sequence
    """
    cdf = mystats.Cdf(weights, label='weights')

    mean, var = mystats.TrimmedMeanVar(weights)
    std = math.sqrt(var)
    ...
    xs, ps = mystats.RenderNormalCdf(mean, std, xmin, xmax)
    myplots.Plot(xs, ps, label='model', linewidth=4, color='0.8')
    myplots.Cdf(cdf)
...
def MakeNormalPlot(weights):
    ...
    weights: sequence
    """
    mean, var = mystats.TrimmedMeanVar(weights, p=0.01)
    std = math.sqrt(var)

    xs = [-5, 5]
    xs, ys = mystats.FitLine(xs, mean, std)
    myplots.Plot(xs, ys, color='0.8', label='model')

    xs, ys = mystats.NormalProbability(weights)
    myplots.Plot(xs, ys, label='weights')
...
def MakeFigures(df):
    ...
    myplots.Config(xlabel='adult weight (kg)', ylabel='CDF')

    myplots.SubPlot(2)
    MakeNormalModel(log_weights)
    myplots.Config(xlabel='adult weight (log10 kg)')

    myplots.Save(root='brfss_weight')

    # make normal probability plots on linear and log scales
    myplots.PrePlot(cols=2)
    MakeNormalPlot(weights)
    myplots.Config(xlabel='z', ylabel='weights (kg)')

    myplots.SubPlot(2)
    MakeNormalPlot(log_weights)
    myplots.Config(xlabel='z', ylabel='weights (log10 kg)')

    myplots.Save(root='brfss_weight_normal')
