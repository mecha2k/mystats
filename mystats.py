...
class Cdf:
    ...
    def __repr__(self):
        ...
        if self.label == DEFAULT_LABEL:
            ...
            return "%s(%s, %s, %s)" % (cls, str(self.xs), str(self.ps), repr(self.label))
...
def NormalProbabilityPlot(sample, fit_color="0.8", **options):
    ...
    myplots.Plot(*fit, color=fit_color, label="model")

    xs, ys = NormalProbability(sample)
    myplots.Plot(xs, ys, **options)
...
class HypothesisTest(object):
    ...
    def PlotCdf(self, label=None):
        ...
        def VertLine(x):
            ...
            myplots.Plot([x, x], [0, 1], color="0.8")

        VertLine(self.actual)
        myplots.Cdf(self.test_cdf, label=label)
